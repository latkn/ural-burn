-- Аттестация только на сервере: ответы проверяются в БД, код сертификата выдаётся только при успехе.
-- Публичный register_certificate_code удалён — иначе можно было бы записать любой код без прохождения теста.

-- Ключи правильных ответов (индекс варианта 0..2) — ОБЯЗАНЫ совпадать с
-- onboarding/src/data/attestationQuestions.ts → knowledgeQuestions[].correct
-- При смене вопросов/ответов обновите и этот массив v_correct, и порог (11 * 0.8).
-- Текущая версия: 11 вопросов, все correct = 1.
drop function if exists public.register_certificate_code(text);
...
begin
  ...
  if v_count < v_need then
    return jsonb_build_object(
      ...
      'message', 'Тест не пройден. Нужно не менее 80% правильных ответов.'
    );
  end if;
...
comment on function public.submit_attestation(integer[], boolean) is
  'Проверка ответов аттестации и выдача кода сертификата (без ПДн).';
